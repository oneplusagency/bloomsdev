...
class bannerPriceAdmin extends Controller
{
    /**
    ...
    public function __construct()
    {
        ...
        $this->banners = new DB\Jig\Mapper( $this->db, 'price-banner.json' );
    }
    ...
    public function banner()
    {
        ...
        if ( isset( $_POST["action"] ) ) {
            ...
            if ( $action == "fetch" ) {
                ...
                // $results = $this->db->read('price-banner.json');
                ...
                foreach ( $results as $n => $img ) {
                    ...
                    if ( $img['src'] ) {
                        ...
                        <img src="'.PRICE_BANNER_DIR.$img['src'].'" height="150" class="img" />
                        </td>
            ...
            if ( $action == "insert" ) {
                ...
                if ( $error_code == 200 ) {
                    ...
                    $db          = new DB\Jig( 'app/data/', DB\Jig::FORMAT_JSON );
                    $banners     = new DB\Jig\Mapper( $db, 'price-banner.json' );
                    $slider_save = $banners;
    ...
    public function setPositionDrop()
    {
        ...
        if ( !empty( $newOrder ) && is_array( $newOrder ) ) {
            ...
            $db = new DB\Jig( 'app/data/', DB\Jig::FORMAT_JSON );
            $db->drop();
            $slider = new DB\Jig\Mapper( $db, 'price-banner.json' );
