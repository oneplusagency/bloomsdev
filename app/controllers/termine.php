...
class termine extends Controller
{
    public function index()
    {
        ...
        $this->f3->set('title', "Termine");
        $this->f3->set('view', 'termine.html');
        ...
        $addscripts[] = 'js/layout/termine.js';
    ...
    public function salon()
    {
        if ($this->f3->exists('PARAMS.id')) {
            ...
            $this->f3->set('SESSION.error', 'Termine existiert nicht');
    ...
    public  function select_salon(
        ...
        if (!empty($salons) && is_array($salons)) {
            foreach ($salons as $skey => $sv) {
                //Mannheim, N7, Akademie ID: 31
                if (strtoupper(trim($sv['DisplayName'])) == 'MANNHEIM, N7, AKADEMIE') continue;
                $selected = '';
                if ($salonId == $skey) {
    ...
    public function services()
    {
        ...
        if ($test || $this->f3->get('AJAX')) {
            ...
            $onlyAkedemy = $this->f3->get('GET.onlyAkedemy') == true;
            // $onlyAkedemy = false;

            // var_dump($onlyAkedemy);
            // die;
            ...
            if ($employee_id > 0) {
                ...
                // echo "(Find by salone ID : " . $salonId . ")";
                // Alle Salonmitarbeiter

                $diff = $spackModel->getServicepackBysalon($salonId, $onlyAkedemy);
            }

            // print json_encode($diff);die;
            ...
            if ($diff && ($serv_array = helperblooms::jsJson($diff, true))) {
                ...
                //     // Filter only Akedemy service
                //     $serv_array = array_filter($serv_array, function ($v) {
                //         // var_dump($v);
                //         // die;
                //         return $v['IsOnlineVisibleAsAcademyService'] === true;
                //     });
                // }

                $allServicePckageIds = array_map(function ($ser) {
                    return $ser['ServicePackageId'];
                }, $serv_array);

                // echo 'serv_array : ' . json_encode($serv_array);
                // echo 'allServicePckageIds : ' . json_encode($allServicePckageIds);


                $categoryToServicePackageArray = $spackModel->getCasheCategorieService($onlyAkedemy);
                ...
                if ($onlyAkedemy ==  true) {
                    ...
                    // echo json_encode($serv_array) . PHP_EOL;
                    // die;
                    ...
                    // echo json_encode($orderedCategories) . PHP_EOL;
                    // die;

                    // ksort($orderedCategories);

                    // $categoryToServicePackageArray = $orderedCategories;

                    // echo json_encode($categoryToServicePackageArray) . PHP_EOL;
                    // die;
                    ...
                    array_map(function ($sa) use (&$finalSs) {
                    ...
                    array_map(function ($ctsp) use ($allServicePckageIds, &$finalSs) {

                        $services = array_filter($ctsp['ServicePackages'], function ($s) use ($allServicePckageIds) {
                            // var_dump($s, $allServicePckageIds);
                            $BaseServicePackageId = $s['BaseServicePackageId'];
                            $ServicePackageId = $s['ServicePackageId'];

                            return $ServicePackageId != null &&
                                ($BaseServicePackageId === null || (int) $BaseServicePackageId === 0) &&
                                (in_array($s['ServicePackageId'], $allServicePckageIds));
                        });

                        if (count($services) > 0) {
                            // Only those category and its available services

                            $ctspCopy = $ctsp;
                            unset($ctspCopy['ServicePackages']);

                            $ctspCopy['ServicePackages'] = [];

                            $notSortedService = [];
                            foreach ($services as $_ss) {

                                foreach ($ctsp['ServicePackages'] as $pp) {

                                    if (
                                        isset($pp['ServicePackageId'])   &&
                                        isset($pp['BaseServicePackageId'])   &&
                                        (int) $pp['BaseServicePackageId'] === (int) $_ss['ServicePackageId']
                                    ) {
                                        //if((int) $id[0] === (int) $ServicePackageBasic->ServicePackage->BaseServicePackageId[0]){
                                        $basicId = $pp['ServicePackageId'];
                                        // return $basicId;

                                        $_ss['BaseServicePackageId'] = $basicId;
                                    }
                                }
                                ...
                                if ($_ss['WebSortOrder'] == null) {
                                    ...
                                    $ctspCopy['ServicePackages'][$_ss['WebSortOrder']] = $_ss;
                                }
                                // $ctspCopy['BaseServicePackageId'] = $this->getBasicId($_ss['ServicePackageId'], $services);



                            }

                            ksort($ctspCopy['ServicePackages']);

                            $highestSortingValue =  count(array_keys($ctspCopy['ServicePackages'])) > 0 ? max(array_keys($ctspCopy['ServicePackages'])) : 0;

                            foreach ($notSortedService as $nss) {
                                $highestSortingValue += 1;
                                $ctspCopy['ServicePackages'][$highestSortingValue] = $nss;
                            }


                            // Do sorting here according to WebSortOrder

                            // $ctspCopy['ServicePackages'] = array_merge($ctspCopy['ServicePackages'], $notSortedService);
    ...
    protected function getBasicId($id, $ServicePackages)
    {
        ...
        foreach ($ServicePackages as $pp) {
            ...
            if (
                isset($pp['ServicePackageId'])   &&
                isset($pp['BaseServicePackageId'])   &&
                (int) $pp['BaseServicePackageId'] === (int) $id
            ) {
                //if((int) $id[0] === (int) $ServicePackageBasic->ServicePackage->BaseServicePackageId[0]){
                $basicId = $pp['ServicePackageId'];
                return $basicId;
        ...
        if ($test || $this->f3->get('AJAX')) {
            ...
            if ($test || $this->f3->exists('POST.option_salon')) {
                ...
                */
                ...
                // var_dump($diff, $anfangsdatum, $datum);
                // die;
                ...
                if ($mitarbeiterId) {
                    ...
                    // var_dump($allEmpIds, $timeSalonAllMirabiterOnlyAvailable);
                    // die;
                    ...
                    if (!empty($timeSalonAllMirabiter) && is_array($timeSalonAllMirabiter)) {
                        ...
                        foreach ($timeSalonAllMirabiter as $mi => $timeTableData) {
                            ...
                            // var_dump($timeTableData,  $mitarbeiter);
                            // die;
                            if (!$mitarbeiter) {
                                continue;
                            }

                            $zag_date = $skipUnavailable ? $skipperDates[$key_index] : $zag_date;
                            ...
                            */
                            ...
                            $body .=
                                ...
                                '<div class="termine-data-time-slots ' . ($skipUnavailable ? '' : 'hide d-none') . ' termine-data-price my-0 bg-light text-dark p-1">
                                <p class="align-self-center m-0">' .
                                $wochentag .
                                '</p>
                                <p class="align-self-center m-0">
                ...
                if ($availableCounter == 0) {
                    ...
                    if ($skipUnavailable) {
                        if ($mitarbeiterId) {
                            $timeTable_dropdown = '<div style="color: #fffff" class="mb-1" id="no-data-available"><p>Leider konnte kein freier Termin gefunden werden. \n Bitte wählen Sie eine/n andere/n Mitarbeiter/in aus. Vielen Dank!</p><p>Bitte wählen Sie einen anderen Termin oder eine/n andere/n Mitarbeiter/in aus.</p>' . $checkBox . '</div>' . $timeTable_dropdown;
                        } else {
                            $timeTable_dropdown = '<div style="color: #fffff" class="mb-1" id="no-data-available"><p>Leider konnte kein freier Termin gefunden werden. \n Bitte wählen Sie einen anderen Standort aus. Vielen Dank!</p><p>Bitte wählen Sie einen anderen Termin oder eine/n andere/n Mitarbeiter/in aus.</p>' . $checkBox . '</div>' . $timeTable_dropdown;
                        }
                    } else {

                        $timeTable_dropdown = '<div style="color: #fffff" class="mb-1" id="no-data-available"><p>Leider ist zu dieser Zeit kein Termin verf&uuml;gbar.</p><p>Bitte wählen Sie einen anderen Termin oder eine/n andere/n Mitarbeiter/in aus.</p>' . $checkBox . '</div>' . $timeTable_dropdown;
                    }
                } else {
