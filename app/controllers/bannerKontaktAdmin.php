...
class bannerKontaktAdmin extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->banners = new DB\Jig\Mapper( $this->db, 'kontakt-banner.json' );
    }
    ...
    public function banner()
    {
        ...
        if ( isset( $_POST["action"] ) ) {
            ...
            $where   = bloomArrayHelper::getValueJoom( $_POST, 'type', 'kontakt-banner', 'STRING' );
            ...
            if ( $action == "fetch" ) {
                ...
                foreach ( $results as $n => $img ) {
                    ...
                    if ( $img['src'] ) {
                        ...
                        <img src="'.BANNER_PARENT_URL_DIR.'/kontakt-banner/'.$img['src'].'" height="150" class="img" />
                        </td>
            ...
            if ( $action == "insert" ) {
                ...
                if ( $error_code == 200 ) {
                    ...
                    $db          = new DB\Jig( 'app/data/', DB\Jig::FORMAT_JSON );
                    $banners     = new DB\Jig\Mapper( $db, 'kontakt-banner.json' );
                    $slider_save = $banners;
    ...
    public function setPosition()
    {
        ...
        if ( !empty( $newOrder ) && is_array( $newOrder ) ) {
            // $results = $this->db->read('kontakt-banner.json');
            $db     = new DB\Jig( 'app/data/', DB\Jig::FORMAT_JSON );
            $slider = $this->banners;
    ...
    public function setPositionDrop()
    {
        ...
        if ( !empty( $newOrder ) && is_array( $newOrder ) ) {
            ...
            $db = new DB\Jig( 'app/data/', DB\Jig::FORMAT_JSON );
            $db->drop();
            $slider = new DB\Jig\Mapper( $db, 'kontakt-banner.json' );
