...
class azubikontakt extends Controller
{
    /**
     * @return mixed
     */
    ...
    public function index()
    {
        ...
        $this->f3->set('title', 'Azubi Kontakt');
        $this->f3->set('view', 'azubi-kontakt.html');
        $this->f3->set('classfoot', 'kontaktformular');
        // ADD JS
        $addscripts = [
            'js/layout/validator.min.js',
            'js/layout/azubi-kontakt.js'
        ];
    ...
    public function sendinfo()
    {
         ...
         * @Desc:
         */
        $from_mail = 'bewerbungen@bloom-s.de';

        $max_file_size = 3; // mb

        //$from_mail='info@developservice.de';
        $replyto = '';
        /* @FIX @Date: 2021-09-06 14:38:18
         * @Desc:
         */
        ...
        if (empty($_POST['message'])) {
            ...
            $message = self::smartCrop($message, 1300);
            $message = preg_replace('/[ \t]+/', ' ', preg_replace('/\s*$^\s*/m', "\n\r", $message));
        ...
        if ($errorMSG != '') {
            ...
            $Body .= $phone;
            $Body .= '</p>';
            $Body .= '<p>Warum möchtest Du friseur werden?: ';
            $Body .= $message;
            $Body .= '</p>';
            ...
            $Subject = 'Bewerbung als Auszubildender';
            // $EmailTo = 'bewerbung@bloom-s.de'; Old one
            $EmailTo = ' bewerbungen@bloom-s.de';
            
            $test    = 0;
            if ($test == 2) {
                $EmailTo = 'adrian@1plus-agency.com';
            }

            $replyto = self::noreply_email($this->home_url, 'noreply');
            if (self::validateEmail($replyto) == false) {
                $replyto = 'bewerbungen@bloom-s.de';
            }
    ...
    public function sendEmail(
        $mail_to = 'bewerbungen@bloom-s.de',
        $name_to = 'bloom-s.de',
        ...
        try {
            ...
            if ($test) {
                $mail->addBCC('adrian@1plus-agency.com');
            }
