...
function setModalContentBloom(id) {
    ...
    var termine_url_base = BloombaseUrl + '/akademiebuchung/salon/' + salonid
...
$('.modal').on('click', '.modal-arrow-left', function () {
    if ($(this).data('page') == 'bilder') {
        return;
    }
    setModalContentBloom($(this).data('index'));
});
$('.modal').on('click', '.modal-arrow-right', function () {
...
(function ($) {
    ...
    $(function () {
        $('#termineAjaxformWizard').validate({
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    url: BloombaseUrl + '/akademiebuchung/terminFinden',
                    // timeout: 6000,
                    type: 'POST',
                    processData: false,
                    contentType: false,
    ...
    $(document).ready(function () {
        ...
        $("#servicePackageField").on("change", function (e) {
            ...
            if (valueSelected > 0) {
                ...
                var url = BloombaseUrl + '/akademiebuchung/getServicePackageDescription';
                $.getJSON(url, {
                    format: 'JSON',
    ...
    $('#termine-data').on('click', '.available', function (e) {
        ...
        var url = BloombaseUrl + '/akademiebuchung/confirmTabtwo';
        $.getJSON(url, {
            format: 'JSON',
