(function($) {
    ...
    const salonId = $('#image_salonId').val();
    ...
    function saveOrderbanner_field() {
        ...
        $.ajax({
            url: BloombaseUrl + '/bannerSalonsAdmin/setPosition',
            type: 'post',
            data: {
                ids: data,
            },
    ...
    function interval_field() {
        ...
        conttable.on('click', '.updatebutton', function() {
            ...
            if (value > 0) {
                ...
                $.ajax({
                    url: BloombaseUrl + '/bannerSalonsAdmin/interval',
                    type: 'post',
    ...
    $(document).ready(function() {
        ...
        $('#salon_auswahlen').on("change", function() {
            var url = BloombaseUrl + '/admin/bannersalons/' + $(this).val();
            window.location = url;
        });


        if (salonId) {
            fetch_banner_data(salonId);
        }


        function fetch_banner_data(salonId) {
            var action = 'fetch';
            $.ajax({
                url: BloombaseUrl + '/bannerSalonsAdmin/banner',
                method: 'POST',
                data: { action: action, salonId: salonId },
                success: function(data) {
            ...
            if (image_name == '') {
                ...
                if (
                    ...
                    $.ajax({
                        ...
                        // url: BloombaseUrl + '/bannerSalonsAdmin/banner?test=1',
                        url: BloombaseUrl + '/bannerSalonsAdmin/banner',
                        method: 'POST',
