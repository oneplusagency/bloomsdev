...
class StringUtil
{
	...
	public static function substr($strString, $intNumberOfChars, $strEllipsis=' …')
	{
		...
		if (JString::strlen($strString) <= $intNumberOfChars)
		{
			return $strString;
		}
		...
		{
			$intCharCount += JString::strlen(static::decodeEntities($strChunk));
			...
			// with JString::substr() so the method does not return an empty string.
			if (empty($arrWords))
			{
				$arrWords[] = JString::substr($strChunk, 0, $intNumberOfChars);
			}
	...
	public static function substrHtml($strString, $intNumberOfChars)
	{
		...
		for ($i=0, $c=\count($arrChunks); $i<$c; $i++)
		{
			...
			$intCharCount += JString::strlen(static::decodeEntities($arrChunks[$i]));
	...
	public static function encodeEmail($strString)
	{
		...
		{
			$strEncoded = '';
			$arrCharacters = JString::str_split($strEmail);

			foreach ($arrCharacters as $strCharacter)
			{
				$strEncoded .= sprintf((random_int(0, 1) ? '&#x%X;' : '&#%s;'), JString::ord($strCharacter));
			}
	...
	public static function standardize($strString, $blnPreserveUppercase=false)
	{
		...
		{
			$strString = JString::strtolower($strString);
		}
