 ...
 * @version:    1.0.0
 * @modified:   Tuesday January 21st 2020 6:11:40 pm
...
class blooms extends Controller
{
    ...
    private function banners()
    {
        ...
        $bannerAdmin = new bannerBloomsAdmin();
        ...
        foreach ( $images as $n => $img ) {
            ...
            // false &&
            if ( !empty( $img['src'] ) ) {
                $banners[] = ['type' => 'img', 'src' => BANNER_PARENT_URL_DIR.'/blooms-banner/'.$img['src'], 'interval' => $img['carousel_interval']];
                $yt++;
	...
	public function index()
	{
		...
		$this->f3->set('title', "Blooms");
		$this->f3->set('view', 'blooms.html');
		$this->f3->set('classfoot', 'blooms');
        // ADD JS
        $addscripts = 'js/layout/blooms.js';
        ...
        $kontackt = new kontakt();
        $staff = $kontackt->adminData();
        $this->f3->set( 'STAFFINFO', $staff );    
