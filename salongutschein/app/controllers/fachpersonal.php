 ...
 * @version:    1.0.0
 * @modified:   Wednesday August 19th 2020 4:16:16 pm
...
class fachpersonal extends Controller
{
    ...
    private function banners()
    {
        ...
        $bannerAdmin = new bannerFachkraftAdmin();
        ...
        foreach ($images as $n => $img) {
            ...
            // false &&
            if (!empty($img['src'])) {
                $banners[] = ['type' => 'img', 'src' => BANNER_PARENT_URL_DIR . '/bewerbung-banner/' . $img['src'], 'interval' => $img['carousel_interval']];
                $yt++;
    ...
    public function index()
    {
        ...
        $this->f3->set('title', "Fachpersonal");
        $this->f3->set('view', 'fachpersonal.html');
        $this->f3->set('classfoot', 'bewerbung');
        // ADD JS
        $addscripts = 'js/layout/fachpersonal.js';
