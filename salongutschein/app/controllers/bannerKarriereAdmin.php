 ...
 * @version:    1.0.0
 * @modified:   Friday July 24th 2020 5:29:28 pm
 * @copyright   (c) 2008-2020 Webiprog GmbH, UA. All rights reserved.
 * @license     GNU/GPLv3 http://www.gnu.org/licenses/gpl-3.0.html
 */


class bannerKarriereAdmin extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->banners = new DB\Jig\Mapper( $this->db, 'karriere-banner.json' );
    }
    ...
    public function banner()
    {
        ...
        if ( isset( $_POST["action"] ) ) {
            ...
            $where   = bloomArrayHelper::getValueJoom( $_POST, 'type', 'karriere-banner', 'STRING' );
            ...
            if ( $action == "fetch" ) {
                ...
                foreach ( $results as $n => $img ) {
                    ...
                    if ( $img['src'] ) {
                        ...
                        <img src="'.BANNER_PARENT_URL_DIR.'/karriere-banner/'.$img['src'].'" height="150" class="img" />
                        </td>
            ...
            if ( $action == "insert" ) {
                ...
                if ( $error_code == 200 ) {
                    ...
                    $db          = new DB\Jig( 'app/data/', DB\Jig::FORMAT_JSON );
                    $banners     = new DB\Jig\Mapper( $db, 'karriere-banner.json' );
                    $slider_save = $banners;
    ...
    public function setPosition()
    {
        ...
        if ( !empty( $newOrder ) && is_array( $newOrder ) ) {
            // $results = $this->db->read('karriere-banner.json');
            $db     = new DB\Jig( 'app/data/', DB\Jig::FORMAT_JSON );
            $slider = $this->banners;
    ...
    public function setPositionDrop()
    {
        ...
        if ( !empty( $newOrder ) && is_array( $newOrder ) ) {
            ...
            $db = new DB\Jig( 'app/data/', DB\Jig::FORMAT_JSON );
            $db->drop();
            $slider = new DB\Jig\Mapper( $db, 'karriere-banner.json' );
