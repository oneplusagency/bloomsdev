 ...
 * @version:    1.0.0
 * @modified:   Friday July 24th 2020 4:56:55 pm
...
class bannerAkademieAdmin extends Controller
{
    ...
    public function __construct()
    {
        ...
        $this->banners = new DB\Jig\Mapper( $this->db, 'akademie-banner.json' );
    }
    ...
    public function banner()
    {
        ...
        if ( isset( $_POST["action"] ) ) {
            ...
            $where   = bloomArrayHelper::getValueJoom( $_POST, 'type', 'akademie-banner', 'STRING' );
            ...
            if ( $action == "fetch" ) {
                ...
                // file_put_contents ( ONEPLUS_DIR_PATH."/bannerAkademieAdmin.txt" , var_export( $results , true),  LOCK_EX );
                // exit;
                ...
                foreach ( $results as $n => $img ) {
                    ...
                    if ( $img['src'] ) {
                        ...
                        <img src="'.BANNER_PARENT_URL_DIR.'/akademie-banner/'.$img['src'].'" height="150" class="img" />
                        </td>
            ...
            if ( $action == "insert" ) {
                ...
                if ( $error_code == 200 ) {
                    ...
                    $db          = new DB\Jig( 'app/data/', DB\Jig::FORMAT_JSON );
                    $banners     = new DB\Jig\Mapper( $db, 'akademie-banner.json' );
                    $slider_save = $banners;
    ...
    public function setPosition()
    {
        ...
        if ( !empty( $newOrder ) && is_array( $newOrder ) ) {
            // $results = $this->db->read('akademie-banner.json');
            $db     = new DB\Jig( 'app/data/', DB\Jig::FORMAT_JSON );
            $slider = $this->banners;
    ...
    public function setPositionDrop()
    {
        ...
        if ( !empty( $newOrder ) && is_array( $newOrder ) ) {
            ...
            $db = new DB\Jig( 'app/data/', DB\Jig::FORMAT_JSON );
            $db->drop();
            $slider = new DB\Jig\Mapper( $db, 'akademie-banner.json' );
