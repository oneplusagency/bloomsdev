...
class IniParser {
    ...
    public static function readFile($filename) {
        ...
        if (!function_exists('parse_ini_file')) {
            throw new Exception("parse_ini_file needs to be enabled");
        }
        // @codeCoverageIgnoreEnd

        if (!file_exists($filename)) {
            throw new Exception("Could not find ini file to parse: {$filename}");
        }
        ...
        if ($parsed === false) {
            ...
            throw new Exception("Error reading ini file '{$basename}': {$e['message']}");
        }
     ...
     * @param string $filename
     * @param array  $array
     * @test IniParser writeFile($filename, $array)
     *
     */
    public static function writeFile($filename, $array) {
                    ...
                    foreach ($value as $kkey => $vvalue) {
                        if (is_array($vvalue)) {
                            foreach ($vvalue as $vvvalue) {
                                if (is_array($vvvalue)) {
                                    throw new Exception("Cannot have nested arrays in ini files");
                                }
        ...
        if (@file_put_contents($filename, $to_write) === false) {
            throw new Exception("Could not write ini file {$filename}");
        }
    }
