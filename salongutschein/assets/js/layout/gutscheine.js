    ...
    this.fillTabZusammenfassung = function () {
        ...
        if (counter == 0) {
            ...
            if (isAnyObject(persDataConfirm)) {
                ...
                const amount = parseFloat(persDataConfirm.amount);


                // $("form#persDataConfirm *").removeClass("validation-error-label");
                // $("form#persDataConfirm #phone").hide();
                // $("form#persDataConfirm #diffAddressContainer").hide();
                ...
                if (amount >= 250) {
                    ...
                    $('#adresse-form', tab_gutschein).text(rechnungsadresse_adresse);
                    $('#adresse-form-container').show();

                    $('#ort-form', tab_gutschein).text(rechnungsadresse_ort);
                    $('#ort-form-container', tab_gutschein).show();

                    $('#plz-form', tab_gutschein).text(rechnungsadresse_plz);
                    $('#plz-form-container', tab_gutschein).show();
                } else {

                    $('#adresse-form-container').hide();
                    $('#ort-form-container').hide();
                    $('#plz-form-container').hide();



                }

                // $('#phone-form', tab_gutschein).text(rechnungsadresse_phone);
                ...
                // $('#diffphone-form', tab_gutschein).text(rechnungsadresse_diffphone);
...
jQuery(document).ready(function ($) {
    ...
    $('form#gutschein #amount').change(function (e) {
        betragAmount = parseInt(e.target.value);
    });
    ...
    function SaveSessionPaypal() {
        ...
        let persDataConfirm = Object.assign(gutschein, persData);

        for (const key in persDataConfirm) {
            if (Object.hasOwnProperty.call(persDataConfirm, key)) {
                const element = persDataConfirm[key];
                if (element.length < 1) {
                    delete persDataConfirm[key];
                }
    ...
    var gutscheineWizard = $('#page-gutscheine').bootstrapWizard({
        ...
        onNext: function (tab, navigation, index) {
            ...
            if (index == 1) {
                ...
                $('#pills-gutscheinauswhal .text-md-left').next('p').css('display', 'block');
                $('#pills-gutscheinauswhal .no-padd-form').css('display', 'flex');

                betragAmount = parseInt($('form#gutschein #amount').val());

                $("form label.validation-error-label").remove();
